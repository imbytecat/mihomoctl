...
「启动／停止」只控制本次运行，「开机启动」独立决定整机重启后是否启动代理：手动停止不会关闭自启，关闭自启也不会停止当前代理。UFI 的运行记录按开机标识核验；断电后旧标记不会冒充仍在生效的规则。状态查询只读，过期 PID 和接管记录在下一次启动、停止或卸载时安全清理，配置和日志保留。规则无法读取或所有权无法确认时会明确报错，不假报清理成功。守护恢复依赖自身仍存活，整机掉电需要供电恢复及自启策略处理。
...
新安装生成默认控制地址和随机密钥。「恢复默认」清空自定义覆写，点击「保存并应用」后回到基础管理设置，保留当前控制端口和密钥；需要修改时直接编辑 YAML。TPROXY、DNS 监听、平台绑定和监听保护由管理器维护，不能通过覆写使其失配。覆写含密钥时仍加密上传、加密读取，并以私有 YAML 保存。
