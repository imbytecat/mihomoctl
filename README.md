...
插件通过「概览 / 设置 / 日志」切换页面，运行状态、任务进度与取消入口始终可见，切换不会清空草稿。启动或重启时自动打开「日志」页，任务执行中即可查看 Mihomo 最近输出。UFI 使用约 1 秒短轮询持续刷新，不是服务端推流；最多显示最近 1000 行，支持关键词筛选、ANSI 颜色及错误高亮。向上翻阅或暂停时停止刷新，点击「跟随最新」恢复，离开日志页停止轮询。操作失败时显示具体原因及清理结果；明确的监听端口冲突会提前结束启动并清理。任务详情显示本地时间的开始、结束与耗时；worker 日志统一包含时间、级别、任务 ID，结束记录包含耗时。CLI 可用 `cancel ID` 请求取消并等待清理完成，`cancel ID --no-wait` 只等待取消请求被接收；取消任务不会停止已经运行的 Mihomo。
