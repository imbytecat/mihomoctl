...
- 重启恢复按 [systemd](https://www.freedesktop.org/software/systemd/man/latest/systemctl.html) 的语义分离本次 start/stop、开机 enable/disable 和运行健康；显式 stop 不被守护反向启动，整机重启仅在自启开启时恢复。UFI 通过 boot_id 限定运行记录，进程同时核对 PID、启动时刻、可执行文件和完整 argv；缺少开机证明时仍须逐项核验当前内核资源，不能从文件存在推断存活。capture 来源为实际自有链与路由的存在性，包含不完整残留；status 只读且查询失败明确报错。过期记录在启动／停止／卸载入口清理，跨开机记录不得授权删除当前同名规则。网络脚本仅执行当前 Agent 内嵌版本；旧落盘脚本在确认进程退出并完成清理后删除。Linux 直接使用 systemd 的原生进程状态与启停语义，不另造 PID 标记或空 runtime 目录。不假设 Magisk/UFI 的启动脚本自带进程重启策略，不引入额外守护框架。
